# API for Storing information on GAR for use in eGAR 
...
By default the service runs by default on port 8084. But to change this, add to the configuration the following:
```
--server.port=8080
```
...
The system will connect to a db instance on the same host with the database `dev_gar_api`.
...
```
mvn -Dflyway.url=jdbc:postgresql://localhost/dev_gar_api -Dflyway.user=postgres -Dflyway.password=mysecretpassword flyway:migrate 
```
